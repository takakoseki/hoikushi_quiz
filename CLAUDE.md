...
11. Google OAuth認証でリフレッシュトークンが定期的に失効する場合は、Google Cloud Console の「OAuth 同意画面」で「公開ステータス」が「テスト」になっていないか確認し、必要に応じて「本番環境」に変更すること
12. テスト環境では OAuth リフレッシュトークンが7日で失効する制限がある。より堅牢な運用を目指す場合は、サービスアカウント方式への移行を検討すること（トークン失効なし、手動再認可不要）
