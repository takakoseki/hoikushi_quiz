...
23. 試験情報（出題数・配点・試験時間など）を記載する際は、ユーザーの不利益に直結するため、確実な情報源に基づくこと。特に教育原理と社会的養護は他科目と異なる制度設計のため注意
...
27. 試験制度・出題数・配点などの情報をサイトに記載する場合は、**学習者の不利益に直結するため、確実な公式情報源（文部科学省・試験運営機関）に基づくこと**。特に教育原理と社会的養護は他科目と異なる制度設計のため要確認
28. 複数の科目ページに同一構造のコンテンツ（FAQ等）を横展開する場合、各科目固有の内容に踏み込まないと Googleから内容の薄い重複と判断される。共通テンプレートを避け、科目ごとの個別性を確保してから実装すること
